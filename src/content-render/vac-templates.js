/**
 * VoiceAI Connect — Template System v3
 * 
 * DESIGN PRINCIPLE: Each template must look completely different at
 * Instagram feed scale (~300x375px on phone). Different shape, different
 * weight, different dominant element.
 * 
 * 1. stat_callout    — MASSIVE number fills 50% of screen, tiny text below
 * 2. did_you_know    — Comparison rows with left accent bars, headline below
 * 3. process_steps   — Vertical step diagram, one step highlighted emerald
 * 4. full_graphic    — Left-aligned editorial, huge headline takes 60% of space
 * 5. warning_signs   — Before/after split: red top half, green bottom half
 * 6. split_feature   — Heavy emerald left stripe, numbered items right
 * 7. checklist       — Emerald accent bar across top, check items fill space
 * 8. service_highlight — 2x2 card grid dominates, small headline above
 * 9. brand_intro     — Centered, max whitespace, logo prominent, CTA button
 * 10. LinkedIn       — 1200x628 landscape, headline + logo, minimal
 */

function esc(s) { return (s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

// ── Logo (base64 embedded) ──────────────────────────────────────
const VAC_LOGO_B64 = 'data:image/png;base64,';

const VAC_LOGO = `<img src="${VAC_LOGO_B64}" style="width:56px;height:56px;object-fit:cover;border-radius:14px;"/>`;
const VAC_LOGO_LG = `<img src="${VAC_LOGO_B64}" style="width:80px;height:80px;object-fit:cover;border-radius:18px;"/>`;

const EMERALD = '#10b981';
const RED = '#ef4444';

// ── Shared icons ────────────────────────────────────────────────
const ICON_CHECK = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${EMERALD}" stroke-width="3" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>`;
const ICON_X = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="${RED}" stroke-width="3" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;

// ── Base shell (1080x1350 Instagram) ────────────────────────────
function vacShell(body) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=Space+Mono:wght@400;700&display=swap');
*{margin:0;padding:0;box-sizing:border-box;}
body{width:1080px;height:1350px;overflow:hidden;background:#050505;}
.post{width:1080px;height:1350px;overflow:hidden;position:relative;background:#050505;color:#fafaf9;font-family:'Plus Jakarta Sans',system-ui,sans-serif;}
.mono{font-family:'Space Mono',monospace;letter-spacing:0.05em;}
.glass{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:16px;}
.em{color:${EMERALD};}
</style></head><body>${body}</body></html>`;
}

function highlight(text, words) {
  if (!words?.length || !text) return esc(text||'');
  let r = esc(text);
  words.forEach(w => { r = r.replace(new RegExp(`(${esc(w)})`, 'gi'), `<span class="em">$1</span>`); });
  return r;
}


// ═══════════════════════════════════════════════════════════════════
// 1. STAT CALLOUT — massive number fills center, tiny context below
// Visual: 160px+ stat dominates. Radial glow behind it. Feed shape: BIG NUMBER
// ═══════════════════════════════════════════════════════════════════

function vacStatCallout(content, biz) {
  const items = content.items || [];
  const stats = content.stats || [];
  const headline = esc(content.headline || '');
  const isNegative = content.content_type === 'diy_trap' || content.content_type === 'missed_call_pain';
  const color = isNegative ? RED : EMERALD;

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:80px;">
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:700px;height:700px;border-radius:50%;background:radial-gradient(circle,${color}15 0%,transparent 65%);pointer-events:none;"></div>
    ${content.eyebrow?`<div class="mono" style="font-size:18px;font-weight:700;color:${EMERALD};text-transform:uppercase;letter-spacing:0.12em;margin-bottom:24px;position:relative;z-index:1;">${esc(content.eyebrow)}</div>`:''}
    <div style="font-size:200px;font-weight:900;color:${color};line-height:0.85;letter-spacing:-0.04em;position:relative;z-index:1;">${headline}</div>
    <div style="font-size:28px;color:rgba(250,250,249,0.5);margin-top:24px;line-height:1.4;max-width:600px;position:relative;z-index:1;">${esc(content.subtext)}</div>
    ${stats.length?`<div style="display:flex;gap:24px;margin-top:48px;position:relative;z-index:1;">
      ${stats.map((s,i)=>`<div class="glass" style="padding:28px 40px;text-align:center;">
        <div style="font-size:40px;font-weight:900;color:${i===0?RED:i===stats.length-1?EMERALD:'#fafaf9'};">${esc(s.value)}</div>
        <div style="font-size:17px;color:rgba(250,250,249,0.4);margin-top:8px;">${esc(s.label)}</div>
      </div>`).join('')}
    </div>`:''}
    <div style="position:absolute;bottom:52px;left:50%;transform:translateX(-50%);">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 2. DID YOU KNOW — comparison rows with colored left borders, headline at bottom
// Visual: Stacked rows fill top 60%, bold headline at bottom. Feed shape: ROWS + TEXT
// ═══════════════════════════════════════════════════════════════════

function vacDidYouKnow(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;padding:72px 64px;">
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;gap:16px;">
      ${items.slice(0,4).map((it,i)=>{
        const l = typeof it==='string'?it:it.title||it;
        const sub = typeof it==='object'?(it.subtitle||''):'';
        const isLast = i===Math.min(items.length,4)-1;
        const borderColor = isLast ? EMERALD : 'rgba(255,255,255,0.08)';
        const bg = isLast ? 'rgba(16,185,129,0.06)' : 'rgba(255,255,255,0.02)';
        return`<div style="padding:28px 32px;border-left:4px solid ${borderColor};background:${bg};border-radius:0 16px 16px 0;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
              <div style="font-size:22px;font-weight:700;color:${isLast?'#fafaf9':'rgba(250,250,249,0.7)'};">${esc(l)}</div>
              ${sub?`<div style="font-size:17px;color:rgba(250,250,249,0.4);margin-top:4px;">${esc(sub)}</div>`:''}
            </div>
            ${isLast?`<span class="mono em" style="font-size:18px;font-weight:700;">best</span>`:''}
          </div>
        </div>`;}).join('')}
    </div>
    <div style="margin-top:48px;">
      <div style="font-size:56px;font-weight:900;line-height:1.1;letter-spacing:-0.04em;">${headline}</div>
      ${content.subtext?`<div style="font-size:21px;color:rgba(250,250,249,0.4);margin-top:16px;line-height:1.5;">${esc(content.subtext)}</div>`:''}
    </div>
    <div style="margin-top:auto;padding-top:40px;">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 3. PROCESS STEPS — vertical diagram, one step glows emerald
// Visual: 3-5 connected steps, step 2 pops green. Feed shape: VERTICAL DIAGRAM
// ═══════════════════════════════════════════════════════════════════

function vacProcessSteps(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);
  const hlIdx = items.length === 3 ? 1 : Math.floor(items.length / 2);

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;padding:72px 64px;">
    <div style="flex:0 0 auto;display:flex;flex-direction:column;gap:0;margin-bottom:48px;">
      ${items.slice(0,5).map((it,i)=>{
        const t = typeof it==='string'?it:it.title||it;
        const sub = typeof it==='object'?(it.subtitle||''):'';
        const num = String(i+1).padStart(2,'0');
        const isHl = i===hlIdx;
        const bg = isHl ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)';
        const border = isHl ? `2px solid ${EMERALD}40` : '1px solid rgba(255,255,255,0.06)';
        return`${i>0?`<div style="width:2px;height:24px;background:${i===hlIdx||i===hlIdx+1?EMERALD+'40':'rgba(255,255,255,0.06)'};margin-left:48px;"></div>`:''}
        <div style="padding:24px 32px;background:${bg};border:${border};border-radius:16px;display:flex;align-items:center;gap:24px;">
          <span class="mono" style="font-size:24px;font-weight:700;color:${isHl?EMERALD:'rgba(250,250,249,0.2)'};">${num}</span>
          <div style="flex:1;">
            <div style="font-size:24px;font-weight:700;color:${isHl?EMERALD:'rgba(250,250,249,0.65)'};">${esc(t)}</div>
            ${sub?`<div style="font-size:17px;color:rgba(250,250,249,${isHl?'0.5':'0.3'});margin-top:4px;">${esc(sub)}</div>`:''}
          </div>
          ${isHl?`<div class="mono" style="font-size:13px;color:${EMERALD};border:1px solid ${EMERALD}30;border-radius:6px;padding:6px 14px;text-transform:uppercase;white-space:nowrap;">the gap</div>`:''}
        </div>`;}).join('')}
    </div>
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;text-align:center;">
      <div style="font-size:52px;font-weight:900;line-height:1.1;letter-spacing:-0.04em;">${headline}</div>
      ${content.subtext?`<div style="font-size:20px;color:rgba(250,250,249,0.4);margin-top:16px;line-height:1.5;max-width:700px;margin-left:auto;margin-right:auto;">${esc(content.subtext)}</div>`:''}
    </div>
    <div style="text-align:center;padding-top:40px;">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 4. FULL GRAPHIC — editorial left-aligned, headline takes 60% of screen
// Visual: Huge headline dominates, small subtext, minimal elements. Feed shape: BIG TEXT LEFT
// ═══════════════════════════════════════════════════════════════════

function vacFullGraphic(content, biz) {
  const items = content.items || [];
  const words = (content.headline||'').split(' ');
  let headlineHtml;
  if (words.length >= 3) {
    const mid = Math.ceil(words.length / 2);
    headlineHtml = `<span style="color:rgba(250,250,249,0.35);">${esc(words.slice(0,mid).join(' '))}</span><br/><span style="color:#fafaf9;">${esc(words.slice(mid).join(' '))}</span>`;
  } else {
    headlineHtml = highlight(content.headline, content.highlight_words);
  }

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;padding:72px 64px;">
    <div style="flex:1;display:flex;flex-direction:column;justify-content:center;">
      <div style="font-size:88px;font-weight:900;line-height:1.0;letter-spacing:-0.045em;">${headlineHtml}</div>
      <div style="width:56px;height:4px;background:${EMERALD};border-radius:2px;margin:36px 0;"></div>
      ${content.subtext?`<div style="font-size:28px;font-weight:700;color:${EMERALD};line-height:1.3;">${esc(content.subtext)}</div>`:''}
      ${items.length?`<div style="margin-top:28px;">
        ${items.slice(0,4).map(it=>{const l=typeof it==='string'?it:it.title||it;
        return`<div style="font-size:20px;color:rgba(250,250,249,0.25);margin-bottom:10px;">${esc(l)}</div>`;}).join('')}
      </div>`:''}
    </div>
    <div style="display:flex;justify-content:space-between;align-items:flex-end;">
      ${VAC_LOGO}
      <div class="mono" style="font-size:16px;color:rgba(250,250,249,0.15);">myvoiceaiconnect.com</div>
    </div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 5. WARNING SIGNS — before/after comparison, red top → green bottom
// Visual: Two distinct zones. Top = problem (dark+red), Bottom = solution (dark+green). Feed shape: TWO-TONE SPLIT
// ═══════════════════════════════════════════════════════════════════

function vacWarningSigns(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);
  const mid = Math.ceil(items.length / 2);
  const problems = items.slice(0, mid);
  const solutions = items.slice(mid);

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;">
    <div style="flex:1;background:linear-gradient(180deg,rgba(239,68,68,0.06) 0%,#050505 100%);padding:72px 64px;display:flex;flex-direction:column;justify-content:flex-end;">
      <div style="font-size:56px;font-weight:900;line-height:1.1;letter-spacing:-0.04em;text-align:center;margin-bottom:40px;">${headline}</div>
      <div style="width:56px;height:3px;background:${EMERALD};border-radius:2px;margin:0 auto 36px;"></div>
      <div style="display:flex;flex-direction:column;gap:14px;max-width:800px;margin:0 auto;width:100%;">
        ${problems.slice(0,2).map(it=>{const l=typeof it==='string'?it:it.title||it;
        return`<div style="padding:22px 28px;background:rgba(239,68,68,0.04);border:1px solid rgba(239,68,68,0.12);border-radius:14px;display:flex;align-items:center;gap:18px;">
          <div style="width:36px;height:36px;border-radius:10px;background:rgba(239,68,68,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;">${ICON_X}</div>
          <span style="font-size:21px;color:rgba(250,250,249,0.6);">${esc(l)}</span>
        </div>`;}).join('')}
      </div>
    </div>
    <div style="flex:1;background:linear-gradient(0deg,rgba(16,185,129,0.06) 0%,#050505 100%);padding:36px 64px 72px;display:flex;flex-direction:column;justify-content:flex-start;">
      <div style="display:flex;flex-direction:column;gap:14px;max-width:800px;margin:0 auto;width:100%;">
        ${solutions.slice(0,2).map(it=>{const l=typeof it==='string'?it:it.title||it;
        return`<div style="padding:22px 28px;background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);border-radius:14px;display:flex;align-items:center;gap:18px;">
          <div style="width:36px;height:36px;border-radius:10px;background:rgba(16,185,129,0.12);display:flex;align-items:center;justify-content:center;flex-shrink:0;">${ICON_CHECK}</div>
          <span style="font-size:21px;color:#fafaf9;font-weight:600;">${esc(l)}</span>
        </div>`;}).join('')}
      </div>
      ${content.subtext?`<div style="font-size:26px;font-weight:700;color:${EMERALD};margin-top:40px;text-align:center;">${esc(content.subtext)}</div>`:''}
      <div style="margin-top:auto;text-align:center;padding-top:32px;">${VAC_LOGO}</div>
    </div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 6. SPLIT FEATURE — thick emerald left stripe, numbered items right
// Visual: Bold emerald bar on left 8%, content right. Feed shape: GREEN STRIPE + CARDS
// ═══════════════════════════════════════════════════════════════════

function vacSplitFeature(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);

  return vacShell(`<div class="post" style="display:flex;">
    <div style="width:12px;background:linear-gradient(180deg,${EMERALD},${EMERALD}40);flex-shrink:0;"></div>
    <div style="flex:1;padding:72px 56px;display:flex;flex-direction:column;">
      <div style="font-size:72px;font-weight:900;line-height:1.05;letter-spacing:-0.045em;">${headline}</div>
      ${content.subtext?`<div style="font-size:21px;color:rgba(250,250,249,0.4);margin-top:14px;">${esc(content.subtext)}</div>`:''}
      <div style="display:flex;flex-direction:column;gap:16px;margin-top:40px;flex:1;justify-content:center;">
        ${items.slice(0,4).map((it,i)=>{
          const t = typeof it==='string'?it:it.title||it;
          const sub = typeof it==='object'?(it.subtitle||''):'';
          const num = String(i+1).padStart(2,'0');
          return`<div class="glass" style="padding:28px 32px;display:flex;align-items:center;gap:24px;">
            <span class="mono" style="font-size:22px;font-weight:700;color:${EMERALD}80;">${num}</span>
            <div>
              <div style="font-size:24px;font-weight:700;color:rgba(250,250,249,0.85);">${esc(t)}</div>
              ${sub?`<div style="font-size:17px;color:rgba(250,250,249,0.4);margin-top:4px;">${esc(sub)}</div>`:''}
            </div>
          </div>`;}).join('')}
      </div>
      <div style="padding-top:36px;">${VAC_LOGO}</div>
    </div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 7. CHECKLIST — emerald bar across top, check items fill the space
// Visual: Bold color bar top, dark body with checks. Feed shape: GREEN BAR + CHECKS
// ═══════════════════════════════════════════════════════════════════

function vacChecklist(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;">
    <div style="background:${EMERALD};padding:48px 64px;flex-shrink:0;">
      <div style="font-size:56px;font-weight:900;line-height:1.05;letter-spacing:-0.04em;color:#050505;">${esc(content.headline)}</div>
      ${content.subtext?`<div style="font-size:21px;color:rgba(5,5,5,0.6);margin-top:12px;">${esc(content.subtext)}</div>`:''}
    </div>
    <div style="flex:1;padding:48px 64px;display:flex;flex-direction:column;justify-content:center;">
      <div style="display:flex;flex-direction:column;gap:0;">
        ${items.slice(0,6).map((it,i)=>{
          const t = typeof it==='string'?it:it.title||it;
          const sub = typeof it==='object'?(it.subtitle||''):'';
          return`<div style="display:flex;align-items:center;gap:20px;padding:24px 0;${i>0?'border-top:1px solid rgba(255,255,255,0.04);':''}">
            <div style="width:32px;height:32px;border-radius:9px;background:rgba(16,185,129,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;">${ICON_CHECK}</div>
            <div>
              <span style="font-size:22px;font-weight:700;color:#fafaf9;">${esc(t)}</span>
              ${sub?`<span style="font-size:18px;color:rgba(250,250,249,0.4);margin-left:14px;">${esc(sub)}</span>`:''}
            </div>
          </div>`;}).join('')}
      </div>
    </div>
    <div style="padding:0 64px 52px;">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 8. SERVICE HIGHLIGHT — 2x2 card grid dominates, small headline above
// Visual: Grid of 4 cards fills 70% of space. Feed shape: CARD GRID
// ═══════════════════════════════════════════════════════════════════

function vacServiceHighlight(content, biz) {
  const items = content.items || [];
  const headline = esc(content.headline || '');
  const icons = [
    `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="${EMERALD}" stroke-width="1.8"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72"/></svg>`,
    `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="${EMERALD}" stroke-width="1.8"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
    `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="${EMERALD}" stroke-width="1.8"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>`,
    `<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="${EMERALD}" stroke-width="1.8"><path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z"/></svg>`,
  ];

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;align-items:center;padding:72px 56px;">
    <div style="font-size:52px;font-weight:900;letter-spacing:-0.04em;text-align:center;line-height:1.1;">${headline}</div>
    <div style="width:48px;height:3px;background:${EMERALD};border-radius:2px;margin:32px 0 40px;"></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;width:100%;flex:1;align-content:center;">
      ${items.slice(0,4).map((it,i)=>{
        const t = typeof it==='string'?it:it.title||it;
        const sub = typeof it==='object'?(it.subtitle||''):'';
        return`<div class="glass" style="padding:40px 32px;text-align:center;display:flex;flex-direction:column;align-items:center;">
          <div style="width:52px;height:52px;border-radius:14px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.12);display:flex;align-items:center;justify-content:center;margin-bottom:20px;">${icons[i%4]}</div>
          <div style="font-size:24px;font-weight:800;color:#fafaf9;margin-bottom:10px;">${esc(t)}</div>
          ${sub?`<div style="font-size:17px;color:rgba(250,250,249,0.4);line-height:1.4;">${esc(sub)}</div>`:''}
        </div>`;}).join('')}
    </div>
    ${content.subtext?`<div style="font-size:20px;color:rgba(250,250,249,0.4);font-style:italic;margin-top:36px;text-align:center;">${esc(content.subtext)}</div>`:''}
    <div style="margin-top:auto;padding-top:36px;">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 9. BRAND INTRO — centered, maximum whitespace, logo prominent, CTA
// Visual: Very airy. Logo big, headline centered, pill CTA. Feed shape: CENTERED + CLEAN
// ═══════════════════════════════════════════════════════════════════

function vacBrandIntro(content, biz) {
  const items = content.items || [];
  const headline = highlight(content.headline, content.highlight_words);

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:80px;">
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,${EMERALD}08 0%,transparent 65%);pointer-events:none;"></div>
    ${VAC_LOGO_LG}
    <div style="font-size:22px;font-weight:700;color:rgba(250,250,249,0.7);margin-top:20px;position:relative;z-index:1;">VoiceAI Connect</div>
    <div style="width:48px;height:3px;background:${EMERALD};border-radius:2px;margin:32px 0;"></div>
    <div style="font-size:68px;font-weight:900;line-height:1.05;letter-spacing:-0.045em;max-width:800px;position:relative;z-index:1;">${headline}</div>
    ${content.subtext?`<div style="font-size:23px;color:rgba(250,250,249,0.45);margin-top:20px;line-height:1.5;max-width:640px;">${esc(content.subtext)}</div>`:''}
    ${items.length?`<div style="margin-top:40px;display:flex;flex-direction:column;gap:12px;align-items:flex-start;text-align:left;">
      ${items.slice(0,3).map(it=>{const l=typeof it==='string'?it:it.title||it;
      return`<div style="display:flex;align-items:center;gap:14px;">
        <div style="width:24px;height:24px;border-radius:50%;background:rgba(16,185,129,0.12);display:flex;align-items:center;justify-content:center;">${ICON_CHECK}</div>
        <span style="font-size:21px;color:rgba(250,250,249,0.65);">${esc(l)}</span>
      </div>`;}).join('')}
    </div>`:''}
    ${content.cta_line2?`<div style="margin-top:48px;padding:22px 56px;border-radius:999px;background:#fafaf9;color:#050505;font-size:24px;font-weight:700;position:relative;z-index:1;">
      <span style="display:flex;align-items:center;gap:10px;"><span style="width:10px;height:10px;border-radius:50%;background:${EMERALD};"></span> ${esc((content.cta_line1||'')+' '+(content.cta_line2||''))}</span>
    </div>`:''}
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// 10. REVIEW SHOWCASE — case study with stat cards
// Visual: Eyebrow + headline centered, stat row, quote. Feed shape: STATS + QUOTE
// ═══════════════════════════════════════════════════════════════════

function vacReviewShowcase(content, biz) {
  const reviews = content.reviews || [];
  const stats = content.stats || [];
  const headline = esc(content.headline || '');

  return vacShell(`<div class="post" style="display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:80px;">
    ${content.eyebrow?`<div class="mono" style="font-size:17px;font-weight:700;color:${EMERALD};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:20px;">${esc(content.eyebrow)}</div>`:''}
    <div style="font-size:48px;font-weight:900;letter-spacing:-0.04em;line-height:1.1;max-width:800px;">${headline}</div>
    <div style="width:48px;height:3px;background:${EMERALD};border-radius:2px;margin:32px 0;"></div>
    ${stats.length?`<div style="display:flex;gap:24px;margin-bottom:40px;">
      ${stats.map((s,i)=>`<div class="glass" style="padding:28px 40px;text-align:center;min-width:180px;">
        <div style="font-size:40px;font-weight:900;color:${i===0?RED:i===stats.length-1?EMERALD:'#fafaf9'};">${esc(s.value)}</div>
        <div style="font-size:17px;color:rgba(250,250,249,0.4);margin-top:8px;">${esc(s.label)}</div>
      </div>`).join('')}
    </div>`:''}
    ${reviews.length?reviews.slice(0,1).map(r=>`<div style="max-width:700px;">
      <div style="font-size:22px;color:rgba(250,250,249,0.5);font-style:italic;line-height:1.6;">"${esc(r.text)}"</div>
      <div style="font-size:17px;color:rgba(250,250,249,0.3);margin-top:12px;">— ${esc(r.author)}</div>
    </div>`).join(''):''}
    <div style="margin-top:auto;padding-top:40px;">${VAC_LOGO}</div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// LINKEDIN — 1200x628 landscape, simple headline + logo
// ═══════════════════════════════════════════════════════════════════

function vacLinkedInShell(body) {
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=Space+Mono:wght@400;700&display=swap');
*{margin:0;padding:0;box-sizing:border-box;}
body{width:1200px;height:628px;overflow:hidden;background:#050505;}
.post{width:1200px;height:628px;overflow:hidden;position:relative;background:#050505;color:#fafaf9;font-family:'Plus Jakarta Sans',system-ui,sans-serif;}
.em{color:${EMERALD};}
</style></head><body>${body}</body></html>`;
}

function vacLinkedIn(content, biz) {
  const headline = highlight(content.headline, content.highlight_words);

  return vacLinkedInShell(`<div class="post" style="display:flex;align-items:center;padding:0 80px;">
    <div style="position:absolute;left:30%;top:50%;transform:translate(-50%,-50%);width:400px;height:400px;border-radius:50%;background:radial-gradient(circle,${EMERALD}08 0%,transparent 70%);pointer-events:none;"></div>
    <div style="flex:1;position:relative;z-index:1;">
      <div style="font-size:52px;font-weight:900;line-height:1.1;letter-spacing:-0.04em;max-width:800px;">${headline}</div>
      ${content.subtext?`<div style="font-size:20px;color:rgba(250,250,249,0.45);margin-top:16px;line-height:1.5;max-width:640px;">${esc(content.subtext)}</div>`:''}
    </div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:12px;flex-shrink:0;margin-left:40px;">
      ${VAC_LOGO}
    </div>
    <div style="position:absolute;bottom:0;left:0;right:0;height:3px;background:linear-gradient(90deg,${EMERALD},transparent);"></div>
  </div>`);
}


// ═══════════════════════════════════════════════════════════════════
// REGISTRY
// ═══════════════════════════════════════════════════════════════════

const VAC_TEMPLATES = {
  stat_callout:      vacStatCallout,
  did_you_know:      vacDidYouKnow,
  process_steps:     vacProcessSteps,
  full_graphic:      vacFullGraphic,
  warning_signs:     vacWarningSigns,
  split_feature:     vacSplitFeature,
  checklist:         vacChecklist,
  service_highlight: vacServiceHighlight,
  brand_intro:       vacBrandIntro,
  review_showcase:   vacReviewShowcase,
  photo_hero:        vacBrandIntro,
  offer_coupon:      vacStatCallout,
};

function renderVacTemplate(templateId, content, biz, options = {}) {
  if (options.platform === 'linkedin') {
    return vacLinkedIn(content, biz);
  }
  const fn = VAC_TEMPLATES[templateId] || vacFullGraphic;
  return fn(content, biz);
}

module.exports = { VAC_TEMPLATES, renderVacTemplate };